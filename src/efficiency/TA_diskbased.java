...
//	public int[][] sorted_list;
	...
	public String sorted_list_file;
...
//	private BufferedReader reader_lists;
	...
	public TA_diskbased(String items_file, String sorted_list) {

		this.items_file = items_file;
		this.sorted_list_file=sorted_list;
...
//	}
	...
	public Item[] querysquential(double[] query, int k) {
		...
		while (iscontinue) {
			...
			for (int t = 0; t < this.K; t++) {
				...
				int item =Data_reader_writer.load_sortedlist(this.sorted_list_file, t, pointers[t]);

				pointers[t] += 1;

				if (!resultset.contains(item)) {
					double score = computescore(query, Data_reader_writer.load_item(this.items_file, item));
	...
	public Item[] query(double[] query, int k) {
		...
		for (int i = 0; i < this.K; i++) {
			...
			int item = Data_reader_writer.load_sortedlist(this.sorted_list_file, i, 0);
			double score = computescore(query, Data_reader_writer.load_item(this.items_file, item));
		...
		while (iscontinue) {
			...
			int item =Data_reader_writer.load_sortedlist(this.sorted_list_file, list_id, pointers[list_id]);
			...
			if (pointers[list_id] < this.V) {
				...
				int itemid = Data_reader_writer.load_sortedlist(this.sorted_list_file, list_id, pointers[list_id]);
				double s = computescore(query, Data_reader_writer.load_item(this.items_file, itemid));
				Item diz = new Item(list_id, s);
				rankedattributes.add(diz);
		...
		while (iscontinue) {
			...
			if (pointers[list_id] < this.V) {
				...
				while (resultset.contains(itemid)) {
					...
					itemid = Data_reader_writer.load_sortedlist(this.sorted_list_file, list_id, pointers[list_id]);
				}
	...
	public double computeThreshold(double[] query, int[] pointers) {
		...
		double max = 0;
		for (int i = 0; i < query.length; i++) {
			int item =Data_reader_writer.load_sortedlist(this.sorted_list_file, i, pointers[i]);
			max += query[i] * Data_reader_writer.load_item(this.items_file, item)[i];
		}
	...
	public Item[] LinearScan(double[] query, int k) {
		...
		for (int i = 0; i < this.V; i++) {
			double score = computescore(query, Data_reader_writer.load_item(this.items_file, i));
