...
public class DividerWithNearbyitems {
	...
	public static void main(String[] args) {
		DividerWithNearbyitems obj = new DividerWithNearbyitems();
...
//					sb.deleteCharAt(sb.length()-1);
	 ...
	 */
	public void find_nearby(int v, ArrayList<Integer> items){
		double lat1=this.locations[v].getFirst();
		double lon1=this.locations[v].getSec();
		for(int vid=0; vid<V;vid++){
			if(vid==v)
				continue;
			PairDouble p=this.locations[vid];
			double dis=Distance.getDistance(lat1, lon1, p.getFirst(), p.getSec());
			if(dis<=Paras.distance_threshold)
				items.add(vid);
		}
	}
}
