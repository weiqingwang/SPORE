...
//	private ArrayList<Pair> hitPairs;
	...
	public RecommenderNew(ArrayList<TestCases> joblist, int index) {
...
//		this.hitPairs = new ArrayList<Pair>();
	...
	public void run() {
		...
		for (TestCases query : joblist) {
			...
			ArrayList<Integer> totalItems = sp.nearbyItems.get(targetV);
			HashSet<Integer> ratedItems = sp.Duv[uq];
			ArrayList<Integer> unratedItems = new ArrayList<Integer>();// store
																		// the
																		// unratedItems
																		// in
																		// total
																		// items
			for (int vid : totalItems) {
				if (!ratedItems.contains(vid))
					unratedItems.add(vid);
			}
