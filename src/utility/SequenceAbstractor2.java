package utility;
/**
 * the outputP function is used to find the old transition sequence corresponding to the tex copied at 24/08
 */
 ...
 * 
 */
public class SequenceAbstractor2 {
	private int U = 2189;// Foursquare
	private int V = 19079;
	...
	private final static double deltaT = 1;
	private final static int numP = 3;

	public SequenceAbstractor2() {
	...
	public static void main(String[] args) {
		SequenceAbstractor2 sa = new SequenceAbstractor2();
	...
	public void order() {
		for (int u = 0; u < U; u++) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (this.dataset.equalsIgnoreCase("foursquare")) {
	...
	public void outputP(String souFile, String tarFile) {
		...
		try {
			...
			while ((str = br.readLine()) != null) {
				...
				for (int i = 0; i < s; i++) {
					Date scandate = seqtsu.get(i);
					int scanvid = seqvsu.get(i);
					if (i == 0 && scandate.equals(date) && scanvid == vid) {
						break;
					}
					if (i == 0) {
						prevsu.add(scanvid);
					...
					inter = this.getDayInter(scandate, predate);
					if (scandate.equals(date) && scanvid == vid && inter <= deltaT) {
						break;
					}
					if (scandate.equals(date) && scanvid == vid && inter > deltaT) {
						prevsu = new ArrayList<Integer>();
