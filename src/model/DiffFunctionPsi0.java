...
public class DiffFunctionPsi0 implements DiffFunction {
	...
	public double valueAt(double[] arg0) {
		...
		for (int z = 0; z < Paras.K; z++) {
			...
			for (int v = 0; v < sp.V; v++) {
				double temp = sp.psi0[z] + sp.psitopic[z][v];
				double sumtemp=sp.dzv[z][v] * (Math.log(gammad) - temp)*this.normal;
				sum += sumtemp;
			}
		}
		return sum;
	...
	public double[] derivativeAt(double[] arg0) {
		...
		for (int v = 0; v < arg0.length; v++) {
			...
			double gamma = 0;
			for (int z = 0; z < Paras.K; z++) {
				gamma += (sp.dz[z] * this.getGamma(z, v));
			}
			r[v] = (gamma - dv);
			if(Math.abs(r[v])>max){
				max=Math.abs(r[v]);
	...
	public double getGammaN(int v, int z) {
		...
		// infer alphan
		double exp = 0;
		exp += (this.psi0[v] + sp.psitopic[z][v]);
		gamman = Math.exp(exp);
		return gamman;
	}
